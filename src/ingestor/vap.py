...
class VAP:
    ...
    """
    def __init__(self, strategy: str = "silero", ducking_level: float = 0.2,
                 rms_threshold: float = 0.05, window_duration: float = 1.0):
        """
        ...
        self.vad_sample_rate = 16000  # Silero expects 16kHz
        ...
        if self.strategy == "silero":
            try:
                ...
                print(f"Warning: Failed to load Silero VAD ({e}), falling back to 'rms'.")
                self.strategy = "rms"
    ...
    def _prepare_audio(self, chunk: MediaChunk) -> torch.Tensor:
        ...
        # PyAV fltp format: to_ndarray() gives (channels, samples) per frame.
        # After .tobytes() and concatenation, the layout is:
        # [ch0_frame0, ch1_frame0, ch0_frame1, ch1_frame1, ...]
        # So we reshape to (N_frames_total_samples, channels) isn't quite right.
        # Actually each frame contributes channels*samples floats in row-major order.
        # The simplest robust approach: reshape to (-1, channels) and average.
        ...
        # Resample from source rate (e.g. 48kHz) to 16kHz for VAD
    ...
    def process_chunk(self, chunk: MediaChunk) -> List[float]:
        ...
        # Step 1: Run VAD on 1-second audio windows
        ...
        # Step 2: Map each frame to its corresponding second's mask
        frame_masks = []
        for frame_idx in range(num_frames):
            # Calculate the timestamp of this frame within the chunk
            frame_time = (frame_idx / num_frames) * duration
            # Which 1-second window does it fall into?
            window_idx = min(int(frame_time / self.window_duration), len(second_masks) - 1)
